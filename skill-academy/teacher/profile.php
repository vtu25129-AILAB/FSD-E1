...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_profile') {
        ...
        if (!$errors) {
            db()->prepare("UPDATE users SET full_name = ? WHERE id = ?")->execute([$fullName, $teacherId]);
            $ok = 'Profile updated.';
    ...
    if ($action === 'change_password') {
        ...
        if (!$errors) {
            ...
            if (!$row || !password_verify($current, (string)$row['password_hash'])) {
                ...
                db()->prepare("UPDATE users SET password_hash = ? WHERE id = ?")->execute([password_hash($new, PASSWORD_DEFAULT), $teacherId]);
                $ok = 'Password updated.';
            }
        }
    }
